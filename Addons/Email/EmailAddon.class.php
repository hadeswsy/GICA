 ...
 */
    ...
    class EmailAddon extends Addon{
        ...
        public $admin_list = array(
            'model'=>'Config',		//要查的表
		'fields'=>"MAIL_TYPE,MAIL_SMTP_HOST,MAIL_SMTP_PORT,MAIL_SMTP_USER,MAIL_SMTP_PASS,MAIL_SMTP_CE,FROM_EMAIL",			//要查的字段
		'map' => array('name' => array('in', array(0 => 'MAIL_TYPE', 1 => 'MAIL_SMTP_HOST', 2 => 'MAIL_SMTP_PORT', 3 => 'MAIL_SMTP_USER', 4 => 'MAIL_SMTP_PASS', 5 => 'MAIL_SMTP_CE', 6 => 'FROM_EMAIL'))),				//查询条件, 如果需要可以再插件类的构造方法里动态重置这个属性
		'order'=>'id desc',		//排序,
        );
...
(null, 'FROM_EMAIL', 1, '发件人名称', 5, '', '发件人名称', 1410925495, 1410925495, 1, 'OneThink', 0);
INSERT;
        D()->execute($sql1);
        D()->execute($sql2);
        D()->execute($sql3);
