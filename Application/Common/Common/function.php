...
*/
 function money_upper($money = 0){
    ...
    $minunits = array('毫', '厘', '分', '角');
    $units = array('圆','拾', '佰', '仟', '万', '拾', '佰', '仟', '亿', '拾', '佰', '仟', '兆', '拾', '佰', '仟');
    ...
    if($decimal != 0){
        ...
        for($i = 0; $i < 4; $i++){
            if($decimal[$i] == 0 && $decimal[$i-1] == 0)continue;
    ...
    for($i = 0; $i < $length; $i++){
        if($i > 0 && $integer[$i] == 0 && $integer[$i-1] == 0 && $i % 4 != 0)continue;//当前为0,且小一位也是0,且不在区间单位上,则为空
